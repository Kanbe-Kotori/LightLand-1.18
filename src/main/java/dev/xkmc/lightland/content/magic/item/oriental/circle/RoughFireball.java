...
public class RoughFireball extends DelayedMagic {
    ...
    public static final float BASIC_DAMAGE = 8F;
    ...
    public InteractionResultHolder<ItemStack> wandUse(Level level, Player player, InteractionHand hand) {
        ...
        if (stack != null && stack.getItem() instanceof OrientalWand wand) {
            ItemStack circle = OrientalWand.getCircle(stack);

            Vec3 xAxis = player.getLookAngle().normalize();
            Vec3 shootPos = player.position()
                    .add(0, player.getEyeHeight(), 0)
                    .add(xAxis.scale(2));
            int charge = getDelay(circle);
            float damage = BASIC_DAMAGE * getDamageModifier(circle) * calcTotalBuff(wand.elemental_buffs);

            EntityRoughFireball ball = EntityRoughFireball.create(level, player, charge, damage);
            ball.setPos(shootPos);
            ball.shoot(xAxis.x,xAxis.y,xAxis.z,0,1);
